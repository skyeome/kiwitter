...
function Home({userObj}){
  ...
  useEffect(()=>{
    onSnapshot(collection(dbService,"kiweets"),(doc)=>{
  ...
  return <div>
    <form onSubmit={onSubmit}>
      <input value={kiweet} onChange={onChange} type="text" placeholder="What's on your mind?" maxLength={120}/>
      <input type="submit" value="Kiweet" />
    </form>
    <div>
      {kiweets.length > 0 && kiweets.map(kwt=><div key={kwt.id}>{kwt.text}</div>)}
    </div>
  </div>
}
